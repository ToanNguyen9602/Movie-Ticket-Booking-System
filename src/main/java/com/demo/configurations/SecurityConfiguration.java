...
public class SecurityConfiguration {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		final String[] permittedUrls = { "/", "/home/**", "/blog/**", "/cinema/**", "/account/**", "/film/**",
				"/bookseat/**", "/contact/**", "/images/**", "/user/**", "/food/**" };
		...
		return http.cors(cor -> cor.disable()).csrf(cs -> cs.disable()).authorizeHttpRequests(auth -> {
			auth.requestMatchers(new AntPathRequestMatcher("/")).permitAll()
					...
					.requestMatchers(new AntPathRequestMatcher("/ajax/**")).permitAll().requestMatchers("/images/**")
					.permitAll().requestMatchers(new AntPathRequestMatcher("/admin/register")).hasRole("ADMIN")
					.requestMatchers(new AntPathRequestMatcher("/admin/**")).hasAnyRole("STAFF", "ADMIN");
		}).formLogin(formLogin -> {
			formLogin.loginPage("/user/login").loginProcessingUrl("/account/process-login")

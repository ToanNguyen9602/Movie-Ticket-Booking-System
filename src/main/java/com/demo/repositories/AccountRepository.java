...
public interface AccountRepository extends CrudRepository<Account, Integer> {
	...
	@Query("SELECT SUM(bd.price) FROM Account a JOIN a.bookings b JOIN b.bookingDetailses bd WHERE a.id = :accountId")
	public Integer sumBookingPricesByAccountId(@Param("accountId") Integer accountId);

	@Query("SELECT SUM(bd.price) FROM Account a JOIN a.bookings b JOIN b.foodBookingDetailses bd WHERE a.id = :accountId")
	...
	@Query("SELECT a, SUM(bd.price) + SUM(fbd.price) AS totalSum " + "FROM Account a " + "LEFT JOIN a.bookings b "
			+ "LEFT JOIN b.bookingDetailses bd " + "LEFT JOIN b.foodBookingDetailses fbd " + "GROUP BY a "
			+ "ORDER BY totalSum DESC LIMIT 5")
	public List<Account> findTop5AccountsByTotalPriceWithLimit();
	
    @Query("SELECT SUM(bd.price) + SUM(fbd.price) " +
            "FROM BookingDetails bd " +
            "JOIN bd.booking b " +
            "JOIN FoodBookingDetails fbd " +
            "ON bd.id.bookingId = fbd.id.bookingId " +
